...
let arrcit = [...dados.cities]
let arrherois = [...dados.candidates]
let arreleitor = [...dados.election]

let cidades = []

function percentual(votos, presenca) {
    return ((votos / presenca) * 100).toFixed(2)
}

function votacao({ cityId, candidateId, votes }, i, j) {
    if (cityId === arrcit[i].id && candidateId === arrherois[j].id) {
        return votes;
    }
    else {
        return 0;
...
for (let i = 0; i < 5; i++) {
    // arrcit[i].herois = [...arrherois].map(heroi => ({...heroi}))
    arrcit[i].herois = JSON.parse(JSON.stringify(arrherois))
    //problema de referencia:
    //arrcit[i].herois = [...arrherois]
    ...
    for (let j = 0; j < 13; j++) {
        ...
        for (let k = 0; k < arreleitor.length; k++) {
            votos += votacao(arreleitor[k], i, j)
        }

        arrcit[i].herois[j]['voto'] = votos;
        arrcit[i].herois[j]['percentual'] = percentual(votos, arrcit[i].presence)
    }
    arrcit[i]["id2"] = i
    let part = 0
    arrcit[i].herois.forEach(H => {
        if(!H.voto == 0){part++}
    });
    arrcit[i]["participantes"] = part
   // cidades.push(JSON.parse(JSON.stringify(arrcit[i])));
     arrcit[i].herois = arrcit[i].herois.sort((a,b)=> b.voto - a.voto )
    cidades.push(arrcit[i])
